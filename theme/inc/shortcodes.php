 ...
 * @param mixed $template_part Template part.
 * @param mixed $section_class Section class.
 * @param mixed $sort_key Sort key.
...
function gk_person_shortcode( $atts, $template_part, $section_class, $sort_key = 'kr8mb_pers_pos_sortierung', $variant = 'team' ) {
    ...
    $temp = $wp_query;
    // phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited -- Legacy template parts expect the active person query; the original global is restored after rendering.
    ...
    if ( ! empty( $atts['person'] ) ) {
        ...
        $args       = array(
            ...
            'meta_key'  => $sort_key,
            'abteilung' => ( $atts['slug'] ? $atts['slug'] : $atts['abteilung'] ),
        );
    } else {
        $args = array(
            'post_type'      => 'person',
...
function gk_shortcode_abteilung( $atts ) {
    ...
    if ( $zuordnung && 'kreisverband' !== $zuordnung ) {
    ...
    <section class="gk-abteilung">
        ...
        foreach ( $persons as $item ) :
            ...
            $ov_label = '';
            if ( 'kreisverband' !== $ov_slug && isset( $ov_tabs[ $ov_slug ] ) ) {
                $ov_label = $ov_tabs[ $ov_slug ];
            }
			?>
            <a class="gk-abteilung__card" href="<?php echo esc_url( $permalink ); ?>" data-ov="<?php echo esc_attr( $ov_slug ); ?>">
...
function gk_shortcode_gliederungen( $atts ) {
    ...
    foreach ( $ov_terms as $term ) :
        ...
        $filter_type = 'full' === $atts['type'] ? 'ov' : $atts['type'];
        if ( ! empty( $filter_type ) && $type !== $filter_type ) {
			continue;
        }
    ...
    <article class="clearfix">
        ...
        <header class="article-header">
            ...
            if ( $homepage_url ) :
				?>
                <a href="<?php echo esc_url( $homepage_url ); ?>"><?php endif; ?><?php echo esc_html( $term->name ); ?>
                <?php
				if ( $homepage_url ) :
					?>
                </a><?php endif; ?></h2>
            <?php if ( ! $homepage_url ) : ?>
                <p class="ov-placeholder"><?php esc_html_e( 'Im Aufbau', 'neurg-kreisverband' ); ?></p>
            <?php endif; ?>
        </header>
        <section class="entry-content">
            <p class="contact">
                <?php
                if ( ! empty( $contact['www'] ) ) :
					?>
                    <a href="<?php echo esc_url( $contact['www'] ); ?>" title="Website"><i class="fa fa-globe"></i></a><?php endif; ?>
                <?php
                if ( ! empty( $contact['email'] ) ) :
					?>
                    <a href="mailto:<?php echo esc_attr( $contact['email'] ); ?>" title="E-Mail"><i class="fa fa-envelope"></i></a><?php endif; ?>
            </p>
...
function gk_shortcode_parallax( $atts ) {
    ...
    $style = '';
    if ( $atts['hintergrundfarbe'] ) {
		$style .= 'background-color:#' . esc_attr( $atts['hintergrundfarbe'] ) . ';';
    }
    if ( $atts['schriftfarbe'] ) {
		$style .= 'color:#' . esc_attr( $atts['schriftfarbe'] ) . ';';
    }
    if ( $atts['hintergrundbild'] ) {
		$style .= 'background-image:url(' . esc_url( $atts['hintergrundbild'] ) . ');';
    }
