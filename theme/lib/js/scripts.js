            ...
            function (nav) {
				...
				function reflow() {
					...
					for (var j = 0; j < siblingsCount; j++) {
						var sib = siblings[j];
						if (sib === ulContainer) {
							continue; // skip the main menu list.
						}
						if (sib.offsetWidth === 0) {
							continue; // skip hidden elements (e.g. screen-reader headings).
						}
					...
					var itemsCount       = items.length;
					for (var i = breakIndex; i < itemsCount; i++) {
        ...
        function () {
			...
			if (scrollTop > flyinOffset) {
				...
				if (wasHidden) {
					...
					if (flyinNavEl && flyinNavEl._gkReflow) {
						flyinNavEl._gkReflow();
					}
				}
			} else {
    ...
    $( '.kreiskarte-responsive' ).each(
        function () {
			...
			if ($sheet.length) {
				...
				$kr.on(
                    'click',
                    '.gk-ov-map-svg a[data-ov-name]',
                    function (e) {
                        // Keyboard activation and modified clicks retain native link behavior.
                        if ( ! isMobile() || (e.originalEvent && e.originalEvent.detail === 0) || e.ctrlKey || e.metaKey || e.shiftKey || e.altKey) {
                            return;
                        }
				...
				$( document ).on(
                    'keydown',
                    function (e) {
                        if ( ! $sheet.hasClass( 'gk-ov-sheet--open' )) {
                            return;
                        }
                        if (e.key === 'Escape') {
                            e.preventDefault();
                            ...
                            var first = $sheetCta[0];
                            var last  = $sheet.find( '.gk-ov-sheet__close' )[0];
