...
<section class="gk-hero gk-hero--fundraising">
    ...
    <div class="gk-hero__content inner">
        <span class="gk-hero__kicker"><?php echo esc_html( $kv_name ); ?></span>
        <h1 class="gk-hero__title"><?php echo esc_html( $fr_title ? $fr_title : 'Unterstütze grüne Politik vor Ort' ); ?></h1>
        <?php if ( $fr_text ) : ?>
        ...
        <?php if ( $fr_goal > 0 ) : ?>
            <div class="gk-fundraising-bar">
                <div class="gk-fundraising-bar__track">
                    <div class="gk-fundraising-bar__fill" style="width:<?php echo esc_attr( $fr_pct ); ?>%"></div>
                </div>
...
<section class="gk-map" id="ovs">
    <div class="inner">
        ...
        <?php
        // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Renderer escapes data and supplies the intended SVG and interactive HTML.
        echo gk_render_kreiskarte_responsive( array( 'max_width' => '100%' ) );
        ?>
    </div>
</section>
<?php endif; ?>
...
<!-- ╔═══════════════════════════════════════════════════════════════╗
    ...
    ╚═══════════════════════════════════════════════════════════════╝ -->
<?php
if ( gk_homepage_option( 'show_aktuelles', true ) ) :
...
<!-- ╔═══════════════════════════════════════════════════════════════╗
    ...
    ╚═══════════════════════════════════════════════════════════════╝ -->
<?php
if ( gk_homepage_option( 'show_termine', true ) ) :
    $events = new WP_Query(
        array(
			'post_type'      => 'gk_event',
			'posts_per_page' => intval( gk_homepage_option( 'termine_count', 5 ) ),
			// phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_key -- Bounded upcoming-event query requires existing event date metadata.
			'meta_key'       => 'gk_event_start_date',
			'orderby'        => 'meta_value',
			'order'          => 'ASC',
			// phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_query -- Bounded upcoming-event query requires existing event date metadata.
			'meta_query'     => array(
				array(
...
<section class="gk-events">
    <div class="inner">
        ...
        <div class="gk-events__more">
            <a href="<?php echo esc_url( ( get_post_type_archive_link( 'gk_event' ) ? get_post_type_archive_link( 'gk_event' ) : home_url( '/termine/' ) ) ); ?>" class="gk-btn gk-btn--primary">
                Alle Termine &rarr;
            </a>
        </div>
    </div>
